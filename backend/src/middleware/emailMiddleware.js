...
class EmailMiddleware {
    ...
    setupEventListeners() {
        ...
        this.eventEmitter.on('user:registered', this.handleUserRegistration.bind(this));
        this.eventEmitter.on('user:passwordReset', this.handlePasswordReset.bind(this));
        ...
        this.eventEmitter.on('subscription:renewed', this.handleSubscriptionRenewed.bind(this));
        this.eventEmitter.on('subscription:expired', this.handleSubscriptionExpired.bind(this));
    }
    ...
    async handlePaymentCompleted(data) {
        ...
        try {
            await this.emailService.sendPaymentConfirmationEmail(user, paymentDetails);
    ...
    async handleLevelUp(data) {
        const { user, oldLevel, newLevel, levelData } = data;
    ...
    async handleSubscriptionRenewed(data) {
        ...
        try {
            ...
            await this.sendCustomEmail(user.email, 
                'Assinatura Renovada - DespFinance Premium',
                'subscription-renewed',
                {
                    name: user.name,
                    renewalDate: new Date(subscription.renewedAt).toLocaleDateString('pt-BR'),
                    nextBilling: new Date(subscription.nextBilling).toLocaleDateString('pt-BR'),
    ...
    async handleSubscriptionExpired(data) {
        ...
        try {
            ...
            await this.sendCustomEmail(user.email,
                ...
                {
                    name: user.name,
                    expiredDate: new Date(subscription.expiredAt).toLocaleDateString('pt-BR'),
    ...
    getRedeemInstructions(platform) {
        const instructions = {
            'Netflix': '1. Acesse netflix.com/redeem\n2. Insira o código acima\n3. Aproveite seus meses grátis!',
            'Spotify': '1. Acesse spotify.com/redeem\n2. Faça login na sua conta\n3. Insira o código\n4. Confirme o resgate',
            'Amazon Prime': '1. Acesse amazon.com.br/prime\n2. Vá em "Resgatar código"\n3. Digite o código\n4. Ative sua assinatura',
            'Disney+': '1. Acesse disneyplus.com\n2. Clique em "Resgatar"\n3. Insira o código\n4. Crie ou faça login na conta'
        };
        return instructions[platform] || 'Siga as instruções da plataforma para resgatar seu código.';
    }
    ...
    logEmailError(type, recipient, error) {
        ...
        if (process.env.NODE_ENV !== 'production') {
            console.error(`[${timestamp}] Erro ao enviar email ${type} para ${recipient}:`, error.message);
        }
    }
    ...
    emitLevelUp(user, oldLevel, newLevel, levelData) {
        this.eventEmitter.emit('user:levelUp', { user, oldLevel, newLevel, levelData });
    }
